...
BeforeDiscovery {
    ...
    $script:SkipLocal = $script:Skip -or
                        -not (Test-Path (Join-Path $PSScriptRoot '..', '..', 'certs', 'admin.p12'))
}
...
Describe 'Certificate Files Exist on Server' -Tag 'Integration', 'Certificates' {
    ...
    It 'truststore-root.jks exists' -Skip:$script:Skip {
    ...
    It 'truststore-intermediate-ca.jks exists' -Skip:$script:Skip {
        $r = Invoke-TAKSSHCommand -Session $script:SSH `
            -Command 'test -f /opt/tak/certs/files/truststore-intermediate-ca.jks && echo exists'
        $r.Output | Should -Be 'exists'
    }

    It 'truststore-intermediate-ca.p12 exists' -Skip:$script:Skip {
        $r = Invoke-TAKSSHCommand -Session $script:SSH `
            -Command 'test -f /opt/tak/certs/files/truststore-intermediate-ca.p12 && echo exists'
        $r.Output | Should -Be 'exists'
    }

    It 'takserver.jks (server keystore) exists' -Skip:$script:Skip {
        $r = Invoke-TAKSSHCommand -Session $script:SSH `
            -Command 'test -f /opt/tak/certs/files/takserver.jks && echo exists'
        $r.Output | Should -Be 'exists'
    }

    It 'admin.p12 exists in /opt/tak/certs/files/' -Skip:$script:Skip {
        $r = Invoke-TAKSSHCommand -Session $script:SSH `
            -Command 'test -f /opt/tak/certs/files/admin.p12 && echo exists'
        $r.Output | Should -Be 'exists'
    }

    It 'user.p12 exists in /opt/tak/certs/files/' -Skip:$script:Skip {
        $r = Invoke-TAKSSHCommand -Session $script:SSH `
            -Command 'test -f /opt/tak/certs/files/user.p12 && echo exists'
        $r.Output | Should -Be 'exists'
    }

    It 'admin.p12 is accessible from SSH user home directory' -Skip:$script:Skip {
        $r = Invoke-TAKSSHCommand -Session $script:SSH `
...
Describe 'Certificate Metadata Configuration' -Tag 'Integration', 'Certificates' {
    ...
    It 'cert-metadata.sh exists' -Skip:$script:Skip {
        $r = Invoke-TAKSSHCommand -Session $script:SSH `
            -Command 'test -f /opt/tak/certs/cert-metadata.sh && echo exists'
        $r.Output | Should -Be 'exists'
    }

    It 'cert-metadata.sh has a non-empty STATE field' -Skip:$script:Skip {
        $r = Invoke-TAKSSHCommand -Session $script:SSH `
    ...
    It 'cert-metadata.sh has a non-empty CITY field' -Skip:$script:Skip {
        $r = Invoke-TAKSSHCommand -Session $script:SSH `
    ...
    It 'cert-metadata.sh has a non-empty ORGANIZATION field' -Skip:$script:Skip {
        $r = Invoke-TAKSSHCommand -Session $script:SSH `
    ...
    It 'cert-metadata.sh CAPASS is not the default empty value' -Skip:$script:Skip {
        $r = Invoke-TAKSSHCommand -Session $script:SSH `
...
Describe 'PKCS#12 Certificate Cryptographic Validity' -Tag 'Integration', 'Certificates', 'Crypto' {
    ...
    It 'admin.p12 is readable by openssl pkcs12' -Skip:$script:Skip {
    ...
    It 'admin.p12 certificate has not expired' -Skip:$script:Skip {
        $pass = $script:Config.CertPass
...
"@
    ...
    It 'admin.p12 certificate is valid for at least 30 more days' -Skip:$script:Skip {
        $pass = $script:Config.CertPass
...
"@
    ...
    It 'admin certificate subject contains expected CN' -Skip:$script:Skip {
        $pass = $script:Config.CertPass
...
Describe 'Locally Downloaded Certificates' -Tag 'Integration', 'Certificates', 'Local' {
    ...
    BeforeAll {
        # $script:SkipLocal was set as a boolean during BeforeDiscovery; no change needed here.
        # The variable is kept in BeforeAll scope for any runtime use within test bodies.
    }

    It 'admin.p12 exists in local certs/ directory' -Skip:$script:SkipLocal {
        Test-Path $script:AdminP12Local | Should -Be $true
    }

    It 'local admin.p12 is a valid X509 certificate (importable)' -Skip:$script:SkipLocal {
    ...
    It 'local truststore-intermediate-ca.p12 exists' -Skip:$script:SkipLocal {
        Test-Path $script:CAP12Local | Should -Be $true
    }

    It 'local truststore-intermediate-ca.p12 is a valid X509 certificate' -Skip:$script:SkipLocal {
