...
BeforeAll {
    ...
    $script:SkipNetwork = if (-not $script:Config) {
        'TAK_INTEGRATION_HOST is not set — skipping network reachability tests'
    } else {
        $null
    }

    $script:SkipHyperV = if (-not $script:HyperVAvailable) {
        'Hyper-V module not available — run on the Hyper-V host to include these checks'
    } else {
        $null
    }
}
...
Describe 'Hyper-V Host Prerequisites' -Tag 'Integration', 'Preflight', 'VM' {
    ...
    It 'Hyper-V PowerShell module is available' -Skip:($null -ne $script:SkipHyperV) {
        $script:HyperVAvailable | Should -Be $true
    }

    It 'Running as Administrator (required for Hyper-V operations)' -Skip:($null -ne $script:SkipHyperV) {
        $principal = [Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()
        $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator) | Should -Be $true
    }

    It 'At least one External virtual switch exists' -Skip:($null -ne $script:SkipHyperV) {
...
Describe 'Integration Target Network Reachability' -Tag 'Integration', 'Preflight', 'Network' {
    ...
    It 'SSH port (22) is reachable on TAK_INTEGRATION_HOST' -Skip:($null -ne $script:SkipNetwork) {
    ...
    It 'TAK API port (8443) is reachable on TAK_INTEGRATION_HOST' -Skip:($null -ne $script:SkipNetwork) {
    ...
    It 'CoT port (8089) is reachable on TAK_INTEGRATION_HOST' -Skip:($null -ne $script:SkipNetwork) {
    ...
    It 'Certificate enrollment port (8446) is reachable on TAK_INTEGRATION_HOST' -Skip:($null -ne $script:SkipNetwork) {
