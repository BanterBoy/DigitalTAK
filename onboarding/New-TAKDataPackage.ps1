...
.PARAMETER CertPassphrase
    PKCS12 passphrase for the user certificates (SecureString).
    NEVER logged or written to disk outside the final .zip.

.PARAMETER CertDir
    Directory containing the per-user .p12 files.
    Defaults to the folder containing ManifestPath.

.PARAMETER TrustStorePath
    Path to truststore-intermediate-ca.jks or .p12 from the TAK Server.
    If omitted, the script looks for truststore-intermediate-ca.jks alongside
    ManifestPath, then falls back to truststore-intermediate-ca.p12.
...
.PARAMETER TrustStorePassphrase
    Passphrase for the truststore.  Prompted if omitted.
...
# ── Helper: expand SecureString in memory only ────────────────────────────────
...
# Derive srcStoreType from the actual file extension — .p12 requires PKCS12,
# .jks requires JKS.  Never hardcode the store type.
$srcStoreType = if ($jksPath -match '\.p12$') { 'PKCS12' } else { 'JKS' }

# ── Truststore passphrase ──────────────────────────────────────────────────────
if (-not $TrustStorePassphrase) {
    $TrustStorePassphrase = Read-Host -AsSecureString 'Truststore passphrase (intermediate-ca)'
}

# ── Convert truststore -> PKCS12 in a temp dir ───────────────────────────────
...
# ── Output directory ───────────────────────────────────────────────────────────
$resolvedOutputDir = if ($OutputDir) { $OutputDir } else { Join-Path (Get-Location) "dist\$teamName" }
