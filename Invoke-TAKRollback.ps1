...
.DESCRIPTION
    ...
    Deploy-CivTAK.ps1) and restores either the specified snapshot or the
    most recent one.

    Available snapshots (created by Deploy-CivTAK.ps1):
    ...
    You can then re-run Deploy-CivTAK.ps1 to resume from the restored phase,
    or connect manually.
...
.EXAMPLE
    # Restore to most recent deployment snapshot:
...
if (-not $deploySnapshots) {
    ...
    Write-Host "Deploy-CivTAK.ps1 creates snapshots named Phase0-*, Phase2-*, Phase4-*." -ForegroundColor DarkGray
    exit 0
}
...
$targetSnap = if ($SnapshotName) {
    ...
    # Default: most recent deployment snapshot
    $deploySnapshots[0]
}
...
# Indicate which Deploy-CivTAK.ps1 phase will be resumed on next run
...
Write-Host "  Next Deploy-CivTAK.ps1 run will resume from Phase $nextPhase" -ForegroundColor Cyan
Write-Host '═══════════════════════════════════════════════════' -ForegroundColor Green
Write-Host ''
