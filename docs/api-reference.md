...
{: .note }
**TAKServerPS — validated April 2026.** 39 of 46 end-to-end tests pass against a live TAK Server 5.7-RELEASE8 RPM install. All GET cmdlets, `Connect-TAKServer`, user and mission lifecycle are operational. `New-TAKUser` (REST) and `Set-TAKUserGroup` have a known server-side ESAPI bug — see the individual cmdlet entries below. Full results: [Validation Report](../validation-report/).
...
{: .note }
**Validated — April 2026.** 39 of 46 end-to-end tests pass. All GET cmdlets, `Connect-TAKServer`, user lifecycle, and mission management are operational. `New-TAKUser` REST and `Set-TAKUserGroup` have a known server-side bug — see individual cmdlet entries. Full results: [Validation Report](../validation-report/).
...
{: .warning }
**Server-side bug in TAK Server 5.7-RELEASE8.** `POST /Marti/api/users/` throws HTTP 500 (ESAPI `NullPointerException`). The cmdlet is correct. **Workaround:** `sudo java -jar /opt/tak/utils/UserManager.jar usermod -p 'PASSWORD' USERNAME` over SSH. See [Troubleshooting](../troubleshooting/#symptom-new-takuser-or-set-takusergroup-returns-http-500--nullpointerexception).
...
{: .warning }
**Server-side bug in TAK Server 5.7-RELEASE8.** `PUT /user-management/api/update-groups` throws HTTP 500 (ESAPI `NullPointerException`). The cmdlet is correct. **Workaround:** `sudo java -jar /opt/tak/utils/UserManager.jar usermod -g GROUP_NAME USERNAME` over SSH. See [Troubleshooting](../troubleshooting/#symptom-new-takuser-or-set-takusergroup-returns-http-500--nullpointerexception).
...
{: .note }
All GET cmdlets, `Connect-TAKServer`, user and mission lifecycle validated (April 2026). `New-TAKUser` and `Set-TAKUserGroup` have a server-side ESAPI bug. See [Validation Report](../validation-report/).
