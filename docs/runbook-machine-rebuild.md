...
Paperclip runs as a local server process managed by PM2. All company state — agents, tasks, issues, API keys, and secrets — lives in `~\.paperclip\instances\default\`. A full restore requires:

1. Reinstalling prerequisites (Node.js, PM2, paperclipai CLI)
2. Restoring the DigitalTAK repo (ecosystem config lives here)
3. Restoring the Paperclip data directory from backup
4. Starting PM2 and confirming the server is operational
5. Reinstalling the startup persistence mechanism
...
| What | Path | Notes |
|------|------|-------|
| Startup scripts | `%USERPROFILE%\.paperclip\Start-Paperclip.ps1` | Re-created by `Install-PaperclipStartup.ps1` |
| PM2 dump | `%USERPROFILE%\.pm2\dump.pm2` | Regenerated by `pm2 save` |
...
PM2 uses `ecosystem.config.js` from the DigitalTAK repo to run the Paperclip server.
...
Confirm `ecosystem.config.js` is present:

```powershell
Test-Path C:\Users\$env:USERNAME\DigitalTAK\ecosystem.config.js
# Expected: True
```
...
module.exports = {
  apps: [{
    name: "paperclip",
    cwd: "C:\\Users\\<username>\\DigitalTAK",
    script: "C:\\Windows\\System32\\cmd.exe",
    args: ["/c", "npx", "paperclipai", "run"],
    autorestart: true
  }]
}
```
...
{: .note }
Replace `<username>` with the actual Windows username if the `cwd` path in the file does not match the current user's profile.
...
## Step 4 — Start Paperclip via PM2
...
## Step 6 — Install Startup Persistence

Copy the startup scripts from the repo or from backup to their expected paths:

```powershell
# The startup scripts live at:
# %USERPROFILE%\.paperclip\Start-Paperclip.ps1
# %USERPROFILE%\.paperclip\Install-PaperclipStartup.ps1
```

If restoring from backup, copy both `.ps1` files to `%USERPROFILE%\.paperclip\`. If starting fresh, they can be obtained from the company's configuration exports.

Run the installer (no elevation required for the default Startup Folder method):

```powershell
& "$env:USERPROFILE\.paperclip\Install-PaperclipStartup.ps1"
```

Expected output:

```
Startup Folder entry installed: C:\Users\<username>\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\PaperclipServer.vbs
Paperclip will launch silently at next logon.
```

Test without rebooting:

```powershell
wscript "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\PaperclipServer.vbs"
```

#### Optional: Task Scheduler method (elevated)

For a more resilient setup that survives restart-on-failure:

```powershell
# Run from an elevated (Administrator) PowerShell prompt
& "$env:USERPROFILE\.paperclip\Install-PaperclipStartup.ps1" -Method TaskScheduler
```
...
### Port 3100 not listening after `pm2 start`

1. Check PM2 logs: `pm2 logs paperclip --lines 100`
2. Check if `npx paperclipai run` resolves: run it directly in a terminal to see startup errors.
3. Ensure Node.js is on PATH for the user account that runs PM2.
...
### PM2 process shows status `errored`

```powershell
pm2 logs paperclip --err --lines 100
```

Common causes:
- `npx` not resolving (Node.js not on PATH for the process user)
...
| PM2 logs | `%USERPROFILE%\.pm2\logs\paperclip-out.log` |
| PM2 dump | `%USERPROFILE%\.pm2\dump.pm2` |
| Startup launcher | `%USERPROFILE%\.paperclip\Start-Paperclip.ps1` |
| Startup installer | `%USERPROFILE%\.paperclip\Install-PaperclipStartup.ps1` |
| PM2 ecosystem config | `%USERPROFILE%\DigitalTAK\ecosystem.config.js` |
| Startup VBS (active) | `%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\PaperclipServer.vbs` |
