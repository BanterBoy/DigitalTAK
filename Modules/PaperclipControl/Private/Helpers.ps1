# Private helpers for PaperclipControl module.
...
function Test-PaperclipPort {
    <#
    .SYNOPSIS
        Returns $true if something is listening on the Paperclip service port.
    #>
    [CmdletBinding()]
    [OutputType([bool])]
    param(
...
function Invoke-Pm2Command {
    <#
    .SYNOPSIS
        Runs a pm2 command and returns an object with ExitCode and Output.
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
