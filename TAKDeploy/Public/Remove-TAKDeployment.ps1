...
.DESCRIPTION
    ...
    This cmdlet is idempotent — re-running when already cleaned up is safe.
...
.EXAMPLE
    PS> Remove-TAKDeployment

    Destroys the VM and removes all local artefacts using defaults.

.EXAMPLE
    PS> Remove-TAKDeployment -Organization 'LEIGH-SERVICES' -CAName 'TAK-CA'

    Remove with a custom org/CA name matching deployment parameters.

.EXAMPLE
    PS> $cred = Get-Credential -UserName 'atak'
    PS> Remove-TAKDeployment -UninstallGuest -Credential $cred

    Clean uninstall from guest before VM destruction.

.EXAMPLE
    PS> Remove-TAKDeployment -VMName 'CivTAK-Prod' -VHDPath 'D:\VMs\CivTAK-Prod\CivTAK-Prod.vhdx'

    Remove a named VM with an explicit VHDX path.
#>
function Remove-TAKDeployment {
    ...
    Write-Host '  Remove-TAKDeployment — Full Teardown' -ForegroundColor Red
    ...
    Write-Host "  Remove-TAKDeployment complete — '$VMName' fully removed." -ForegroundColor Green
    Write-Host '═══════════════════════════════════════════════════════' -ForegroundColor Green
    Write-Host ''
}
