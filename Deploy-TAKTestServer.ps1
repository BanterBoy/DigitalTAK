...
<#
.SYNOPSIS
    Creates a TAKServer Hyper-V VM, installs Rocky Linux 9 unattended, deploys
    TAK Server, runs validation tests, and generates a deployment report.

.DESCRIPTION
    End-to-end automation for a TAK Server test deployment:
    ...
    scratch with an unattended kickstart install — no manual input required.
...
.PARAMETER State
    State for certificate subject. Defaults to ESSEX.

.PARAMETER City
    City for certificate subject. Defaults to SOUTHEND-ON-SEA.

.PARAMETER Organization
    Organization for certificate subject. Defaults to LEIGH-SERVICES.

.PARAMETER OrganizationalUnit
    OU for certificate subject. Defaults to IT-DEPARTMENT.

.PARAMETER CAName
    Certificate Authority name. Defaults to TAK-CA.
...
.EXAMPLE
    ...
    .\Deploy-TAKTestServer.ps1 -Credential $cred -RootPassword $rootPw -KeystorePassword $ksPw
...
param (
    ...
    [string] $State               = 'ESSEX',
    [string] $City                = 'SOUTHEND-ON-SEA',
    [string] $Organization        = 'LEIGH-SERVICES',
...
if ($resumePhase -le 2) {
    ...
    $tests += Test-Remote -Name 'Firewall has 9001/tcp open' `
        -Command 'sudo firewall-cmd --list-ports' -Expected '9001/tcp'
    ...
    $tests += Test-Remote -Name 'CA certificate exists' `
        -Command "test -f /opt/tak/certs/files/${CAName}-truststore-root.jks && echo exists" -Expected 'exists'
    ...
    # Test 16: HTTPS responds on 8443
    $tests += Test-Remote -Name 'WebTAK HTTPS responds on 8443' `
        -Command "curl -sk -o /dev/null -w '%{http_code}' https://localhost:8443/" -Expected '(200|302|401|403)'

    # Test 17: /opt/tak/certs/cert-metadata.sh is patched
    $tests += Test-Remote -Name 'cert-metadata.sh has correct State' `
        -Command "grep 'STATE=' /opt/tak/certs/cert-metadata.sh" -Expected $State
...
sudo cp /opt/tak/certs/files/admin.p12 /opt/tak/certs/files/user.p12 /opt/tak/certs/files/truststore-intermediate-ca.p12 /home/atak/
sudo chown atak:atak /home/atak/*.p12
sudo chmod 600 /home/atak/*.p12
'@ -TimeOut 10 -ErrorAction SilentlyContinue | Out-Null
    ...
    Import-PfxCertificate -FilePath $intermediateP12Path -CertStoreLocation 'Cert:\CurrentUser\Root' `
        -Password $keystorePassword -Exportable | Out-Null
    ...
    $reportLines += "| Federation | https://${VMIpAddress}:9001 (TLS) |"
    $reportLines += ''
